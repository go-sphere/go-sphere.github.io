...
- [`httpx`](https://github.com/go-sphere/httpx) — router/context/handler interfaces, a `stdx` (net/http) engine, plus adapters for Gin, Fiber, Echo, and Hertz
- `server/httpz` — JSON envelopes plus `WithJson`, `WithSSE`, and `AbortWithJsonError` on top of `httpx`

Official templates use the `stdx` adapter over plain `net/http` as the default engine, but generated code talks to `httpx`, not to a framework context type.
...
2. **Request arrives** at an `httpx` adapter (net/http `stdx` by default)
...
- **Custom Router Types**: swap the default net/http engine for Gin, Fiber, Echo, or Hertz via `httpx` adapters and [`protoc-gen-sphere`](https://github.com/go-sphere/protoc-gen-sphere) `router_type` / `context_type` flags
