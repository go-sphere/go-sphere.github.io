...
Use `httpz.SetDefaultErrorParser` in the template (see `internal/pkg/render/errors.go`) to map validation and persistence errors before the default `httpx.ParseError` fallback. Non-empty parser messages reach the client (joined protovalidate text); Ent not-found/constraint mappings should use generic status text so schema names stay off the wire. Official templates also install `httpz.AbortWithJsonError` through `stdx.WithErrorHandler` so middleware failures use the same envelope as `WithJson`.
